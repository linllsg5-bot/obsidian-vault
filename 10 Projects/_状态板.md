...
- **状态**：Phase 0B（Codex Runtime Adapter 探针；尚未进入正式数据模型实现）
...
- **当前推进**：`C:\Users\lnp\Documents\Playground\cognos\phase0\` 已建立最小 Runtime Adapter、thread 状态存储和无网络恢复测试；官方 `openai-codex 0.144.4` 已装入隔离 `.venv`，3 项测试通过
- **未确认**：正式数据模型、`subject_level`、JSONL/SQLite/Markdown 权威关系；详见 [[10 Projects/Cognos/当前架构]]
- **旧原型**：`cognos-workspace` 已废弃（死模板、假角色、无法真正对话）
- **卡点**：ChatGPT device-code 登录与真实 thread 往返待完成；公开 Git 历史仍有密钥样式内容。ex.1—3 按小林决定保留，不做删除
- **下一步**：完成两次独立进程往返，确认 thread ID 相同且能取回第一轮标记；随后进入上下文连续性回放，不进入旧 Phase 1 五对象模型路线
