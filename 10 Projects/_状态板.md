...
- **当前推进**：`C:\Users\lnp\Documents\Playground\cognos\phase0\` 已建立最小 Runtime Adapter；官方 `openai-codex 0.144.4` 以标准 ChatGPT 浏览器 OAuth 登录。两次独立进程恢复同一 thread `019ff791-3c1a-76a1-b6ec-4b1125e2388a`，第二轮正确取回 `COGNOS-PHASE0-A`。本轮折返后，adapter 除 `thread_id` 与最终文本外开始保留精确 `turn_id`；4 项无网络测试通过
...
- **卡点**：需要用 ex.2 数学探针观察“什么材料必须重新进入一次真实沟通”，还不能预先把答案定成某种 `ContextSnapshot` 形状；公开 Git 历史仍有密钥样式内容。ex.1—3 按小林决定保留，不做删除
- **下一步**：从现有 Runtime Adapter 做 ex.2 回放探针，同时记录每次真实发生的 thread/turn 来源与实际缺失；先看现有结构怎样被撞开，再决定需要什么上下文视图或持久结构
- **相关**：[[AI/当前主线]] 主线 D · [[10 Projects/Cognos/总图与生成关系]] · [[10 Projects/Cognos/实施计划]] · [[10 Projects/Cognos/来源索引]]
