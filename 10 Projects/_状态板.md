...
### Cognos — 第一数字承载（2026-08-04 立项，2026-08-13 更新）
- **入口**：[[10 Projects/Cognos/_入口]]
- **状态**：Phase 0B 已验收；停在 Phase 1 上下文连续性回放入口，尚未进入正式数据模型实现
...
- **当前推进**：`C:\Users\lnp\Documents\Playground\cognos\phase0\` 已建立最小 Runtime Adapter；官方 `openai-codex 0.144.4` 以标准 ChatGPT 浏览器 OAuth 登录。两次独立进程恢复同一 thread `019ff791-3c1a-76a1-b6ec-4b1125e2388a`，第二轮正确取回 `COGNOS-PHASE0-A`；3 项测试通过
- **未确认**：正式数据模型、`subject_level`、JSONL/SQLite/Markdown 权威关系；详见 [[10 Projects/Cognos/当前架构]]
- **旧原型**：`cognos-workspace` 已废弃（死模板、假角色、无法真正对话）
- **卡点**：Phase 1 仍需定义可见的 `ContextSnapshot` 与 ex.2 数学探针回放输入；公开 Git 历史仍有密钥样式内容。ex.1—3 按小林决定保留，不做删除
- **下一步**：进入 Phase 1，只实现上下文快照与 ex.2 回放验收，不进入旧 Phase 1 五对象模型路线
