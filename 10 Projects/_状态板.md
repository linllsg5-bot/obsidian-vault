...
- **状态**：第一发生场单模型闭环已验收并通过多轮真实实测；正式进入**第一组织切片**（同一发生场内多 Participant 共在与定向邀请）
- **一句话**：以认识论思想为内核，让新存在以组织为第一阶段形态，在软件中获得第一种数字承载
- **已确认约束**：关系依附空间；调度是主体联合；学习是完整展开；架构逐步在沟通中设计；发生先于认识；$\text{Persistent History} \neq \text{Position} \neq \text{Viewpoint}$
- **当前运行计划（2026-08-15 重定）**：
  - **A. 第一组织切片（当前主工程）**：从“一个 assistant + 切换 runtime”走向“小林 + Codex + Gemini 多个独立 participant 共在同一发生场”，各自保持独立连续性，支持最简手动定向邀请（绝不做自动轮询）。
  - **B. 真实使用观察线**：持续沉淀真实发生与未决张力，不抢先编码。
  - **C. 基础设施与可触发探针**：按现实阻力触发（Source Query / Migration / Position 探针），不按排期强推。
- **当前推进**：`Playground/cognos/web/` 已支持 Codex 与 Gemini 双运行时；正在重构 Participant 与 Runtime 解耦，推进第一组织切片
...
- **下一步**：实现第一组织切片：在 `Playground/cognos/web/` 中建立多 Participant 共在与定向邀请机制，完成三主体接续与独立连续性验收。
