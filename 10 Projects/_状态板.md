...
- **状态**：Phase 0（项目承载层已建立；协议 / 项目实现边界已拆；安全收口与根目录重复清理待完成）
- **一句话**：以认识论思想为内核，让新存在以组织为第一阶段形态，在软件中获得第一种数字承载
- **技术栈**：Python FastAPI + SQLite + HTML/JS
- **数据模型**：Space / EntityObject / ObjectInSpace / Relation / Movement
- **数据权威（待运行验证）**：JSONL 事件流 / SQLite 可重建索引 / Markdown 人类可读投影，详见 [[10 Projects/Cognos/当前架构]]
- **旧原型**：`cognos-workspace` 已废弃（死模板、假角色、无法真正对话）
- **卡点**：公开 Git 历史仍有密钥样式内容；ex.1—ex.3 根目录副本与归档 raw 完全重复
- **下一步**：先完成安全收口与根目录清理，再进入 Phase 1（models.py + database.py + 重建测试）
- **相关**：[[AI/当前主线]] 主线 D · [[10 Projects/Cognos/实施计划]] · [[10 Projects/Cognos/来源索引]]
