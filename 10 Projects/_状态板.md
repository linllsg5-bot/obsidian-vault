...
### Cognos — 第一数字承载（2026-08-04 立项，2026-08-13 折返更新）
- **入口**：[[10 Projects/Cognos/_入口]]
- **状态**：Phase 0B 已验收；正在 Phase 1 从现有实现内部做认识折返，尚未进入正式数据模型实现
...
- **当前推进**：`C:\Users\lnp\Documents\Playground\cognos\phase0\` 已建立最小 Runtime Adapter；官方 `openai-codex 0.144.4` 以标准 ChatGPT 浏览器 OAuth 登录。两次独立进程恢复同一 thread `019ff791-3c1a-76a1-b6ec-4b1125e2388a`，第二轮正确取回 `COGNOS-PHASE0-A`。本轮折返后，adapter 除 `thread_id` 与最终文本外开始保留精确 `turn_id`。`phase1/replay_probe.py` 已能按原文件哈希+行号逐字回放并追加保存实际输入、输出和 thread/turn；共 7 项无网络测试通过
...
- **卡点**：真实基线将把 ex.2 `raw.md` 第 183 行发送给外部 Codex；安全审查要求小林先对这段具体原文作明确外发授权，因此调用尚未执行、Vault 中也未生成实录。公开 Git 历史仍有密钥样式内容。ex.1—3 按小林决定保留，不做删除
- **下一步**：获明确授权后运行这一条逐字基线，同时增加 `t0 发生 → t1 旧解释 → schema 改变 → t2 新解释` 探针；先看真实断裂，再决定最小结构，不预设 `ContextSnapshot` 或 `UnknownRegion`
