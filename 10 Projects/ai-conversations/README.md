...
> **三层归档：raw 保留第一手证据，标注提供认知标记，index 负责路由进入。**
...
| 文件 | 内容 | 谁读 |
|------|------|------|
| `index.md` | 结构化尾迹：产出、未解决张力、来源入口、给下次 AI 的操作指引 | **第一入口** |
| `标注.md` | 清洗导出噪声 + 说话人 + 认知标记（实/幻/校正/开口/割裂） | index 指到哪，再定向读哪 |
| `raw.md` | 去除凭证后的第一手公开档案 | 需要逐字核查时翻 |
...
### AI 要做的
1. 先做凭证扫描与脱敏，再调用 [[.claude/skills/对话标注/SKILL]]，读 raw → 产出 `标注.md`。
